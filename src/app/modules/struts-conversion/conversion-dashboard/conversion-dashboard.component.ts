...
export class ConversionDashboardComponent implements OnInit, AfterViewInit {
  ...
  // debugger
    ...
    for (const message of this.msg) {
      let partObj = {};
      partObj["text"] = message["content"];
      partsObj.push(partObj);
    }
    ...
    this.isLoading = true;
    // const userPrompt = trimmedMessage;
    const userPrompt = {
        "jsonrpc": "2.0",
        "id": "1234",
        "method": "message/send",
    ...
    // this.http.post<any>('https://orchestrator-agent-ai-999590495492.us-central1.run.app', { prompt: userPrompt }).subscribe(
    this.http.post<any>('https://orchestrator-agent-ai-999590495492.us-central1.run.app', userPrompt).subscribe(
      // async (response) => {
      //   const botMessage = response.response || 'No response from bot.';
      //   let formattedMessage = typeof botMessage === 'string'
      ...
      //       options: response.chartData.options || { responsive: true }
      //     };
      //   }
      ...
      //   newBotMsg.content = this.getSafeHtml(formattedMessage);
      //   newBotMsg.rawText = botMessage;
  
      //   if (response.image_base64) {
      //     newBotMsg.imageBase64 = response.image_base64;
      //     this.scrollToBottom();
      //   }
      ...
      //   const botMessages = results.artifacts[0].parts[0] || 'No response from bot.';
      //   const botMessage = botMessages["text"] || 'No response from bot.';
      //   let formattedMessage =botMessage;// results.artifacts[0].parts;
  ...
  try {
    ...
    // let botMessage = typeof botMessages === 'string'
    //   ? botMessages
    //   : botMessages?.text || 'No response from bot.';
    //   botMessage = botMessage.replace(/```(?:markdown)?(?:\n|<br\/>)+/gi, '').replace(/^#+\s*/gm, '').replace(/^---+\s*/gm, '');
...
botMessage = botMessage
  .replace(/```(?:markdown)?(?:\n|<br\/>)+/gi, '')   // Remove markdown fences
  .replace(/^#+\s*/gm, '')                           // Remove markdown headings
  .replace(/^---+\s*/gm, '')                         // Remove horizontal rules
  .replace(/\n/g, '<br/>')                           // Replace newlines with <br/>
  .replace(/(<br\/>\s*){2,}/gi, '<br/>');            // Normalize multiple <br/> to one
    
    const formattedMessage = typeof botMessage === 'string'
  ? botMessage
      .replace(/```(?:markdown)?(?:\n|<br\/>)+/gi, '')   // Remove markdown fences
      .replace(/^---+\s*/gm, '')                        // Remove horizontal rules
      .replace(/^#+\s*/gm, '')                          // Remove markdown headings
      .replace(/\n/g, '<br/>')                          // Replace newlines with <br/>
      .replace(/(<br\/>\s*){2,}/gi, '<br/>')            // Replace multiple <br/> with single
      .replace(/\*\*/g, '')                             // Remove bold markers
      .replace(/\*/g, '')                               // Remove asterisks
      .replace(/\n```$/, '')                            // Remove trailing ```
  : this.defaultNoResponseFound;
